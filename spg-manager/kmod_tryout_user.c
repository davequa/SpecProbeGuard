...
int main(int argc, char *argv[]){
	...
	printf("INFO: Time is -- %s", asctime(timeinfo));
	clock_gettime(CLOCK_REALTIME, tp);
	printf("INFO: Time in nanoseconds is -- %ld.\n", tp->tv_nsec);
	...
	if(*argv[1] == '1'){
		...
		ret_val = ioctl(file_desc, IOCTL_SPG_KM_SWITCH_KEY_ENABLE_ALL);
		printf("INFO: Enabled all keys.\n");
	}else if(*argv[1] == '5'){
		ret_val = ioctl(file_desc, IOCTL_SPG_KM_SWITCH_KEY_DISABLE_ALL);
		printf("INFO: Disabled all keys.\n");
	}
