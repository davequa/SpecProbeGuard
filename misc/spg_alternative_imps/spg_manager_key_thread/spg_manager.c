...
/* To synchronise the probe handler and the key activation thread. */
atomic_t key_worker_should_wake;

/* The index of the static key we need to change in the worker. */
int static_key_idx;

/* Holds the task information for the thread that handles key activation. */
struct task_struct *key_worker;

int handler_change_key_state(void *param){
	int ret_val, i;
	int64_t time;
	ret_val = 0;
	time = 0;
	...
	while(!kthread_should_stop()){
		/* Check if this thread should have been awoken at all. */
		if(atomic_read(&key_worker_should_wake) == 0)
			goto sleep;

		// printk(KERN_INFO "%s [DEBUG]: KTHREAD woken up.", DEVICE_NAME);
		...
		for(i = 0; i < NUM_BUFFER_ENTRIES; i++){
			...
			if(time < FLUSH_RELOAD_TIMING_THRESHOLD){
				...
				printk(KERN_INFO "%s [DEBUG]: (%ld) KTHREAD enabled static key: %d.",
					DEVICE_NAME, ktime_get_ns(), i);
			}
		}

		// printk(KERN_INFO "%s [DEBUG]: KTHREAD going to sleep.", DEVICE_NAME);
sleep:
		atomic_set(&key_worker_should_wake, 0);
		msleep(1);
	}

	// printk(KERN_INFO "%s [DEBUG]: KTHREAD exiting.", DEVICE_NAME);
...
static void spec_probe_handler(struct perf_event *evt, struct perf_sample_data
	...
	if(atomic_read(&key_worker_should_wake) == 0){
		atomic_set(&key_worker_should_wake, 1);
		wake_up_process(key_worker);
	}
...
void exit_covert_channel_buffer(void){
	if(kthread_stop(key_worker) < 0){
		printk(KERN_INFO "%s [ERROR]: Failed to stop kthread: key_activation_kthread.\n", DEVICE_NAME);
	}
}
...
void init_covert_channel_buffer(void){
	...
	/* Initialise the synchronisation variable for the handler. */
	atomic_set(&key_worker_should_wake, 0);

	/* Creating the thread to activate the keys, when prompted to. */
	key_worker = kthread_create(handler_change_key_state,
		&static_key_idx, "key_activation_kthread");
	if(key_worker == NULL)
		printk(KERN_INFO "%s [ERROR]: Failed to create kthread: key_activation_kthread.\n", DEVICE_NAME);
