 ...
 * components of SpecProbeGuard, a defence against speculative probing (or,
...
#include "/home/daveq/Desktop/kernels/linux-5.18/include/spg_header.h"
...
/* Tasklet used to offload work to from the PREFETCHT1 interrupt handler. */
struct tasklet_struct *spg_tasklet;

/*
 * To synchronise the tasklet with the interrupt handler, which might start
 * handling new interrupts while the tasklet was already scheduled/busy.
 */
atomic_t is_tasklet_scheduled;

/* 
 * Function that is used for tasklets to offload the covert channel checking and
 * activation of static keys to routines not inside the NMI handler context.
 */
void tasklet_check_covert_channel(unsigned long param){
	int i;
	int64_t time;

	/* Check whether this tasklet was supposed to be scheduled. */
	if(atomic_read(&is_tasklet_scheduled) == 0)
		return;
	 ...
	 *
	 * If we find entries in the buffer that were cached, we activate the
	 * static keys corresponding to these entries. Then, the indirect call
	 * sites will be mitigated through (hence, reactive) code transformation
	 * accordingly.
	 */
	for(i = 0; i < NUM_BUFFER_ENTRIES; i++){
		...
		if(time < FLUSH_RELOAD_TIMING_THRESHOLD){
			 ...
			 * handler or tasklet. However, we choose to ignore this, as it
			 * works and does not give us any other problems.
			 */
			...
			printk(KERN_INFO "%s [DEBUG]: (%ld) TASKLET enabled static key: %d.",
				DEVICE_NAME, ktime_get_ns(), i);
		}
	}

	atomic_set(&is_tasklet_scheduled, 0);
}

/*
 * Event handler that will be triggered once a prefetch event occurs. When
 * using the correct counter configuration (sampling, with a period that
 * suffices), every second prefetch event should trigger it (limitation of
 * perf/counters).
 */
static void spec_probe_handler(struct perf_event *evt, struct perf_sample_data
	...
	/*
	 * Schedule a high priority tasklet to take care of the scanning of the
	 * covert channel, and the activation of static keys where necessary.
	 */
	if(atomic_read(&is_tasklet_scheduled) == 0){
		tasklet_hi_schedule(spg_tasklet);
		atomic_set(&is_tasklet_scheduled, 1);
	}
...
void exit_covert_channel(void){
	/* Kill the tasklet and free its memory, if possible. */
	tasklet_kill(spg_tasklet);
	if(spg_tasklet != NULL)
		kfree(spg_tasklet);
}
 ...
 *
 * Further, initialise a tasklet we use to check the covert channel and
 * activate static keys. We offload these things to such a tasklet, rather than
 * do them in the interrupt handler.
 */
int init_covert_channel(void){
	int ret_val, i, j;
	ret_val = 0;
	...
	/* Now set up the tasklet for use from the interrupt handler. */
	spg_tasklet = (struct tasklet_struct *) kmalloc(sizeof(
		struct tasklet_struct), GFP_KERNEL);
	if(spg_tasklet == NULL){
		ret_val = -1;
		goto out;
	}

	tasklet_init(spg_tasklet, tasklet_check_covert_channel, 0);
...
int init_module(void){
	...
	// Write some decent cleanup routines around here, using gotos and such, to
	// ensure that all the stuff that was allocated/set up is cleaned too.

	// Improve upon the comments and functions and all that crap, too.
