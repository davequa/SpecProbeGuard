...
void CodeGenFunction::EmitCheck(
	...
	// SPG: Then, the control flow goes here. (2)
...
CGCallee SPG_IndCall_Callee;
QualType SPG_CalleeType;
ArrayRef<llvm::Constant *> SPG_StaticArgs;
ArrayRef<llvm::Value *> SPG_DynamicArgs;

bool have_callee = false;

/*
 * SPG: Load the register state of the registers relevant to the indirect call
 * (i.e., parameters) prior to an indirect call from the stack, which we pushed
 * there earlier such that we may move the indirect call and call it via a
 * register without causing trouble.
 */
void SPG_load_reg_state(llvm::Module *M, llvm::IRBuilderBase Builder){
	/* Pop the values for the float registers from the stack first. */
	// llvm::InlineAsm *IARegStateLoad = llvm::InlineAsm::get(
	// 	llvm::FunctionType::get(llvm::Type::getVoidTy(M->getContext()),
	// 		{}, false),
	// 	StringRef("pop %xmm7"),
	// 	StringRef("~{xmm7},~{dirflag},~{fpsr},~{flags}"),
	// 	true,
	// 	false,
	// 	llvm::InlineAsm::AD_ATT,
	// 	false);
	// Builder.CreateCall(IARegStateLoad, {});

	// IARegStateLoad = llvm::InlineAsm::get(
	// 	llvm::FunctionType::get(llvm::Type::getVoidTy(M->getContext()),
	// 		{}, false),
	// 	StringRef("pop %xmm6"),
	// 	StringRef("~{xmm6},~{dirflag},~{fpsr},~{flags}"),
	// 	true,
	// 	false,
	// 	llvm::InlineAsm::AD_ATT,
	// 	false);
	...
	// 		{}, false),
	// 	StringRef("pop %xmm5"),
	// 	StringRef("~{xmm5},~{dirflag},~{fpsr},~{flags}"),
	// 	true,
	// 	false,
	// 	llvm::InlineAsm::AD_ATT,
	// 	false);
	...
	// 		{}, false),
	// 	StringRef("pop %xmm4"),
	// 	StringRef("~{xmm4},~{dirflag},~{fpsr},~{flags}"),
	// 	true,
	// 	false,
	// 	llvm::InlineAsm::AD_ATT,
	// 	false);
	...
	// 		{}, false),
	// 	StringRef("pop %xmm3"),
	// 	StringRef("~{xmm3},~{dirflag},~{fpsr},~{flags}"),
	// 	true,
	// 	false,
	// 	llvm::InlineAsm::AD_ATT,
	// 	false);
	...
	// 		{}, false),
	// 	StringRef("pop %xmm2"),
	// 	StringRef("~{xmm2},~{dirflag},~{fpsr},~{flags}"),
	// 	true,
	// 	false,
	// 	llvm::InlineAsm::AD_ATT,
	// 	false);
	...
	// 		{}, false),
	// 	StringRef("pop %xmm1"),
	// 	StringRef("~{xmm1},~{dirflag},~{fpsr},~{flags}"),
	// 	true,
	// 	false,
	// 	llvm::InlineAsm::AD_ATT,
	// 	false);
	...
	// 		{}, false),
	// 	StringRef("pop %xmm0"),
	// 	StringRef("~{xmm0},~{dirflag},~{fpsr},~{flags}"),
	// 	true,
	// 	false,
	// 	llvm::InlineAsm::AD_ATT,
	// 	false);
	...
	/* Now for the general registers. */
	llvm::InlineAsm *IARegStateLoad = llvm::InlineAsm::get(
		llvm::FunctionType::get(llvm::Type::getVoidTy(M->getContext()),
			{}, false),
		StringRef("pop %r9"),
		StringRef("~{r9},~{dirflag},~{fpsr},~{flags}"),
		true,
		false,
		llvm::InlineAsm::AD_ATT,
		false);
	...
	IARegStateLoad = llvm::InlineAsm::get(
		llvm::FunctionType::get(llvm::Type::getVoidTy(M->getContext()),
			{}, false),
		StringRef("pop %r8"),
		StringRef("~{r8},~{dirflag},~{fpsr},~{flags}"),
		true,
		false,
		llvm::InlineAsm::AD_ATT,
		false);
	...
	IARegStateLoad = llvm::InlineAsm::get(
		llvm::FunctionType::get(llvm::Type::getVoidTy(M->getContext()),
			{}, false),
		StringRef("pop %rcx"),
		StringRef("~{rcx},~{dirflag},~{fpsr},~{flags}"),
		true,
		false,
		llvm::InlineAsm::AD_ATT,
		false);
	...
	IARegStateLoad = llvm::InlineAsm::get(
		llvm::FunctionType::get(llvm::Type::getVoidTy(M->getContext()),
			{}, false),
		StringRef("pop %rdx"),
		StringRef("~{rdx},~{dirflag},~{fpsr},~{flags}"),
		true,
		false,
		llvm::InlineAsm::AD_ATT,
		false);
	...
	IARegStateLoad = llvm::InlineAsm::get(
		llvm::FunctionType::get(llvm::Type::getVoidTy(M->getContext()),
			{}, false),
		StringRef("pop %rsi"),
		StringRef("~{rsi},~{dirflag},~{fpsr},~{flags}"),
		true,
		false,
		llvm::InlineAsm::AD_ATT,
		false);
	...
	IARegStateLoad = llvm::InlineAsm::get(
		llvm::FunctionType::get(llvm::Type::getVoidTy(M->getContext()),
			{}, false),
		StringRef("pop %rdi"),
		StringRef("~{rdi},~{dirflag},~{fpsr},~{flags}"),
		true,
		false,
		llvm::InlineAsm::AD_ATT,
		false);
...
/*
 * SPG: Save the register state of the registers relevant to the indirect call
 * (i.e., parameters) prior to an indirect call to the stack. That way, we can
 * move the call without messing up its parameters.
 */
void SPG_save_reg_state(llvm::Module *M, llvm::IRBuilderBase Builder){
	/* General registers, in user-level application calling convention order. */
	llvm::InlineAsm *IARegStateSave = llvm::InlineAsm::get(
			llvm::FunctionType::get(llvm::Type::getVoidTy(M->getContext()),
				{}, false),
			StringRef("push %rdi"),
			StringRef("~{rdi},~{dirflag},~{fpsr},~{flags}"),
			...
			llvm::InlineAsm::AD_ATT,
			false);
	Builder.CreateCall(IARegStateSave, {});

	IARegStateSave = llvm::InlineAsm::get(
		llvm::FunctionType::get(llvm::Type::getVoidTy(M->getContext()),
			{}, false),
		StringRef("push %rsi"),
		StringRef("~{rsi},~{dirflag},~{fpsr},~{flags}"),
	...
	/* Float registers, in normal order. */
	// IARegStateSave = llvm::InlineAsm::get(
	// 	llvm::FunctionType::get(llvm::Type::getVoidTy(M->getContext()),
	// 		{}, false),
	// 	StringRef("push %xmm0"),
...
void CodeGenFunction::EmitCfiSlowPathCheck(
	...
	if(spg_enable_spec_safe_cfi == true){
		// -------------- SPECULATION-SAFE CFI -- RELEVANT (SPG) -------------- //

		llvm::Module *M = &CGM.getModule();

		// Check whether we have an indirect call saved.
		if(have_callee == false){
			return;
		}

		// Reset already for next indirect call we will instrument (later).
		have_callee = false;
		...
		llvm::InlineAsm *IATestq = llvm::InlineAsm::get(
			...
			llvm::InlineAsm::AD_ATT,
			false);
		Builder.CreateCall(IATestq, {Cond});

		// Create a wrapper function to perform the slow path check, which we will
		...
		llvm::Function *TempFunc = llvm::Function::Create(TempFuncType,
			llvm::Function::ExternalLinkage, StringRef("cfi_slow_path_func_wrap"), M);

		llvm::BasicBlock *temp = llvm::BasicBlock::Create(
			getLLVMContext(), "entry", TempFunc);
		...
		// DEBUG!
		llvm::InlineAsm *test = llvm::InlineAsm::get(
			llvm::FunctionType::get(llvm::Type::getVoidTy(M->getContext()),
				{}, false),
			StringRef("lfence"),
		...
		// Set the builder's insert point back to the old location, ensuring no
		...
		// CALL the RAX register to execute either the indirect call or the slow
		// path check wrapper function, depending on the result of the prior CFI
		// check done.
		llvm::InlineAsm *IACall = llvm::InlineAsm::get(
			llvm::FunctionType::get(llvm::Type::getVoidTy(M->getContext()),
				{}, false),
			StringRef("callq *%rax\0A\091:"),
			StringRef("~{memory},~{rax},~{dirflag},~{fpsr},~{flags}"),
			true,
			false,
			llvm::InlineAsm::AD_ATT,
			false);
		Builder.CreateCall(IACall, {});

		// Lastly, we remove the original indirect call from the code by means of a
		// separate pass we run later (i.e., externally).
...
void CodeGenFunction::EmitTrapCheck(llvm::Value *Checked,
	...
	if(spg_enable_spec_safe_cfi == true){
		...
		// Save the register state onto the stack such that we can find back
		// the arguments of the indirect call later. This only works because
		// we ensure that the call's arguments are emitted prior by changing
		// the rest of the CodeGen.
		// SPG_save_reg_state(M, Builder);

		// Create a wrapper function for the ubsantrap intrinsic, which we will
		// insert into a register; through a CMOV instruction, depending on whether
		// the CFI check on the indirect call's function pointer failed, we will
		// replace it with the real indirect call target in this register or not.
		// The ptr in the register will then be called.
		llvm::FunctionType *TempFuncType = llvm::FunctionType::get(
			llvm::Type::getVoidTy(M->getContext()), false);
		llvm::Function *TempFunc = llvm::Function::Create(TempFuncType,
			llvm::Function::ExternalLinkage, StringRef("cfi_trap_func_wrap"), M);
		...
		// Create a call to the ubsantrap intrinsic and insert into the new
		// function's first and only basic block.
		llvm::CallInst *TrapCall = Builder.CreateCall(CGM.getIntrinsic(
			llvm::Intrinsic::ubsantrap), llvm::ConstantInt::get(CGM.Int8Ty,
				CheckHandlerID));

		// Inline the call to the ubsantrap function.
		llvm::InlineFunctionInfo ifi;
		llvm::InlineFunction(*TrapCall, ifi);
		...
		// register we will call later (with either the indirect call's function
		// pointer, or the trap function if the CFI test failed.
		llvm::InlineAsm *IACmovnzq = llvm::InlineAsm::get(
			llvm::FunctionType::get(llvm::Type::getVoidTy(M->getContext()),
				{SPG_IndCall_Callee.getFunctionPointer()->getType()}, false),
			...
			llvm::InlineAsm::AD_ATT,
			false);
		Builder.CreateCall(IACmovnzq, {SPG_IndCall_Callee.getFunctionPointer()});
		...
		// SPG_load_reg_state(M, Builder);

		// CALL the RAX register to execute either the direct call or the trap,
		// depending on the result of the prior CFI check.
		llvm::InlineAsm *IACall = llvm::InlineAsm::get(
			llvm::FunctionType::get(llvm::Type::getVoidTy(M->getContext()),
				{}, false),
			StringRef("callq *%rax\0A\091:"),
		...
		if (!CGM.getCodeGenOpts().OptimizationLevel || !TrapBB) {
			...
			// SPG: Then, the sample attack gadget gets tackled by this here. (3)
...
RValue CodeGenFunction::EmitCall(QualType CalleeType, const CGCallee &OrigCallee,
	...
	if (SanOpts.has(SanitizerKind::CFIICall) &&
		...
		// SPG: Do a bitcast on the indirect call destination, directly after the
		// function pointer was loaded (not called).

		llvm::Value *CalleePtr = Callee.getFunctionPointer();
		llvm::Value *CastedCallee = Builder.CreateBitCast(CalleePtr, Int8PtrTy);

		// SPG: The call to the type check function comes from here. The function
		// pointer to be called (indirectly) is converted to i8*, and checked.
		...
		// Save the indirect call's information somewhere such that we can use it
		// later for our own defence.
		SPG_IndCall_Callee = Callee;
		SPG_CalleeType = CalleeType;
		SPG_StaticArgs = StaticData;
		SPG_DynamicArgs = {CastedCallee, llvm::UndefValue::get(IntPtrTy)};
		have_callee = true;
		
		if (CGM.getCodeGenOpts().SanitizeCfiCrossDso && CrossDsoTypeId) {

			// SPG: Control flow for a simple speculative probing gadget, built
			// WITH cross-DSO support, goes here. (1)
			...
			// SPG: Control flow for a simple speculative probing gadget, built
			// WITHOUT cross-DSO support, goes here. (1)
	...
	// SPG: Emit the arguments of the indirect call.
	...
	// SPG: This call actually puts in the indirect call.
